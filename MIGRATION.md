...
JackettExtend 现仅支持当前 MoviePilot V3，插件版本为 `3.2.19`，由 oexi 独立维护。原始项目作者为 jtcymc，原始项目地址为 <https://github.com/jtcymc/MoviePilot-PluginsV2>。

V2 版 `ProwlarrExtend` 已删除，仓库不再提供 MoviePilot V2 插件；`package.json` 和 `package.v2.json` 也已移除。

V3 实现优先使用当前公开入口：网络适配器来自 `app.sdk.network`，媒体模型来自 `app.sdk.media`，日志、配置、工具和事件来自 `app.sdk.*`，插件基类来自 `app.plugins`，枚举来自当前 `app.schemas`/`app.schemas.types` 合同。站点持久化仍通过 `_site_registry.py` 适配宿主内部的 `app.db.oper.site`，不将该内部接口视为稳定的站点注册 SDK。
