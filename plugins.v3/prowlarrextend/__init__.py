    ...
    def __fetch_indexers(self, config_snapshot: Optional[dict] = None,
        ...
        try:
            ...
            if "json" not in content_type:
                ...
                return None
            # Bound the body before handing it to a JSON decoder.  Prefer the
            # bytes response when available; lightweight host test doubles
            # commonly expose only ``text``.
            raw_content = getattr(ret, "content", None)
            if isinstance(raw_content, (bytes, bytearray, memoryview)):
                body_size = len(raw_content)
            else:
                body_text = getattr(ret, "text", "")
                body_size = len(body_text.encode("utf-8", errors="ignore")) \
                    if isinstance(body_text, str) else 0
            if body_size > self.REST_MAX_JSON_BYTES:
